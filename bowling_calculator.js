...
const rolls = '9- 9- 9- 9- 9- 9- 9- 9- X X--';
...
// break terminal loop if partial game
if (!rollArr[i]) {
endFrame = frame;
} else {
frames.push([]);
...
let partialGame = false;
if (frames.length < 10) {
partialGame = true;
}

// check if frames > 10 and no extra rolls after last frame;
if (frames.length > 10 || frames[frames.length-1].length > 3) {
validGame = false;
}

let score;
validGame == false ? score = 'invalid game' :
partialGame == true ?
score = `Partial Game. Last frame: ${frame} score: ${calculateScore(frames)}`
: score = `Score: ${calculateScore(frames)}`;
...
/** Notes
1. I tried to reduce the amount of conditional logic using a hashmap in line 11
2. Since the complete solution scores
**/
